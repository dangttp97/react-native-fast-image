...
- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps =
        std::make_shared<const FastImageViewProps>();
    _props = defaultProps;
...
- (void)updateProps:(Props::Shared const &)props
  ...
  const auto &newViewProps =
      *std::static_pointer_cast<FastImageViewProps const>(props);

  NSMutableDictionary *imageSourcePropsDict = [NSMutableDictionary new];
  imageSourcePropsDict[@"uri"] =
      RCTNSStringFromStringNilIfEmpty(newViewProps.source.uri);
  NSMutableDictionary *headers = [[NSMutableDictionary alloc] init];
  for (auto &element : newViewProps.source.headers) {
    [headers setValue:RCTNSStringFromString(element.value)
               forKey:RCTNSStringFromString(element.name)];
  }
  if (headers.count > 0) {
  ...
  FFFastImageSource *imageSource =
      [RCTConvert FFFastImageSource:imageSourcePropsDict];
  ...
  // so we call it after updating the props. If the _eventEmitter is not present
  // yet, we postpone the update till it is set in `updateEventEmitter` since we
  // want to send events to JS.
  if (!_eventEmitter) {
...
- (void)updateEventEmitter:
    (const facebook::react::EventEmitter::Shared &)eventEmitter {
  [super updateEventEmitter:eventEmitter];
  assert(
      std::dynamic_pointer_cast<FastImageViewEventEmitter const>(eventEmitter));
  [fastImageView
      setEventEmitter:std::static_pointer_cast<FastImageViewEventEmitter const>(
                          eventEmitter)];
